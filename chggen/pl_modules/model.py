...
class CHGGen_gemnet(BaseModule):
    def __init__(self, 
    ...
    def langevin_dynamics(self, z, ld_kwargs, gt_num_atoms=None, gt_atom_types=None):
        ...
        for sigma in tqdm(self.sigmas, total=self.sigmas.size(0), disable=ld_kwargs.disable_bar):
            ...
            for step in range(ld_kwargs.n_step_each):
                ...
                pred_cart_coord_diff = pred_cart_coord_diff / sigma # make sure before condition or after
                
                # prop_score = gradient(log(conditional_P), cur_cart_coords)
                # pred_cart_coord_diff  += prop_score  
...
class CHGGen(BaseModule):
    def __init__(self, 
        ...
        #hydra.utils.instantiate(self.hparams.decoder)
        self.decoder = decoder = NequipDecoder()
