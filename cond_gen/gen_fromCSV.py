...
if __name__ == "__main__":
    ...
    formula_list = df_.formula.values.tolist()
    volume_list = df_.avg_volume.values.tolist()
    ...
    for chemical_formula, atomic_volume in zip(formula_list, volume_list):

        for num_inpaint in range(args.num_iterations):
            print("**"*5 + "Start generating structures with inpainting" + "**"*5)
            df_, total_denovo, total_inpaint = main(csp= csp, chemical_formula= chemical_formula, atomic_volume = atomic_volume, 
        ...
        with open(ROOT_DIR + '/successful_rate_infor.txt', "w") as file:
            file.write(f"Total number of denovo generation: {total_denovo}\n")
            file.write(f"Total number of inpaint generation: {total_inpaint}\n")
