...
class Interaction_lattice(torch.nn.Module):
    """Equivariant `Interaction` layer from NequIP.
    References:
    - https://arxiv.org/pdf/2101.03164.pdf
    - https://docs.e3nn.org/en/stable/api/nn/models/gate_points_2101.html
    ...
    def forward(self, x, node_attr, edge_index, edge_attr, edge_len_emb): # layer(h_node_x, h_node_z, edge_index, edge_sh, h_edge)
        ...
        node_features = self.lin1(x, node_attr)
        # Pack scalar with node features

        scalar = torch.tensor(17)
        size_0 = node_features[i].size(0)
        scalar_tensor = scalar.expand(node_features[i].size(0), 1)
        combined_features = torch.cat([node_features[i], scalar_tensor], dim=-1)

        # Use combined features in the convolution
        edge_features = self.conv(combined_features, edge_attr, weight=self.mlp(edge_len_emb))
