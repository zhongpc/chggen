...
from chggen.pl_modules.embeddings import MAX_ATOMIC_NUM, KHOT_EMBEDDINGS
from chggen.pl_modules.encoder import CHGNet_encoder_v_2_0
...
class CHGGen(BaseModule):
    def __init__(self, 
        ...
        self.encoder = CHGNet_encoder_v_2_0(return_crystal_feas = True)
        if self.hparams.decoder == 'nequip':
            self.decoder = NequipDecoder()
        elif self.hparams.decoder == 'gemnet':
    ...
    # =================================================================
    def forward(
        self, batch: Batch, teacher_forcing: bool, training,
    ):
