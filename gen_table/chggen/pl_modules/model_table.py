...
class CHGGen(BaseModule):
    def __init__(self, 
        ...
        self.encoder = CHGNet_encoder_old(return_crystal_feas = True)
    ...
    def langevin_dynamics_guidance(self, z, prop_guidance, ld_kwargs, gt_num_atoms=None, gt_atom_types=None):
        ...
        for sigma in tqdm(self.sigmas, total=self.sigmas.size(0), disable=ld_kwargs.disable_bar):
            ...
            for step in range(ld_kwargs.n_step_each):
                ...
                print('cur_frac_coords shape:', cur_frac_coords.shape)
                print('cur_atom_types shape:', cur_atom_types.shape)
    ...
    def forward(self, batch, teacher_forcing, training):
        ...
        pred_cart_coord_diff  = self.decoder(noisy_frac_coords, rand_atom_types, batch.num_atoms, pred_lengths, pred_angles)
