...
class CHGGen(BaseModule):
    def __init__(self, 
                 hparams_dict = {'latent_dim': 256, 'hidden_dim': 128, 'property_dim': 1, 'load_pretrain': True, 'fc_num_layers': 2, 
                                 ...
                                 'max_atoms': 20, 'num_noise_level': 50, 
                                 ...
                                 'decoder': 'nequip',
                                 'train_decoder': True, 'train_property': True, 'train_composition': True, 
        ...
        self.fc_mu =  build_mlp(64, self.hparams.hidden_dim,
                                    1, self.hparams.latent_dim, use_layernorm= True)
        ...
        self.fc_atom_volume = build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim,
    ...
    def encode(self, batch):
        ...
        mu = self.fc_mu(crystal_fea)
        log_var = self.fc_var(crystal_fea)
    ...
    def decode_stats(self, z, gt_num_atoms=None, gt_lengths=None, gt_angles=None,
        ...
        atom_volume = self.predict_atom_volume(z)

        return num_atoms, lengths_and_angles, lengths, angles, composition_per_atom, atom_volume
    

    def decode_volume(self, z,
                      gt_num_atoms = None,
                     box_lengths = None,
                     box_angles = None,
                     teacher_forcing=False):
        ...
        predict the average volume per atom --> good property can be represented by GNN
        use the avg volume to estimate the num_atoms with given lattice (large box)
        """
    ...
    def compute_grad(self, crystal_graph_list, classifier, ld_kwargs):
    ...
    def diffusion_quench_guidance(self, z, prop_guidance, ld_kwargs,
                                  box_lengths = [10, 10, 10], # for lattice lengths a b c # i.e. [10, 10, 10]
                                  box_angles = [90, 90, 90], # for lattice angles alpha beta gammai.e. [90, 90, 90]
        ...
        cos_alpha = np.cos(box_angles[0]/180*np.pi)
        cos_beta = np.cos(box_angles[1]/180*np.pi)
        cos_gamma = np.cos(box_angles[2]/180*np.pi)

        total_volume = box_lengths[0]*box_lengths[1]*box_lengths[2]*np.sqrt(1 - cos_alpha**2 - cos_beta**2 - cos_gamma**2 + 2*cos_alpha*cos_beta*cos_gamma )

        print("Atom volume", atom_volume)

        num_atoms = torch.round(total_volume / atom_volume, decimals= 0)

        num_atoms = torch.tensor(num_atoms, dtype= torch.int64,  device= z.device)
        num_atoms = torch.clamp(num_atoms, min= 1, max = 200,)
        num_atoms = torch.squeeze(num_atoms, -1)

        lengths = torch.tensor(box_lengths, device= z.device).view(1, -1)
        lengths = lengths.repeat_interleave(len(num_atoms), dim=0)
        angles = torch.tensor(box_angles, device= z.device).view(1, -1)
        angles = angles.repeat_interleave(len(num_atoms), dim = 0)

        _, _, lengths_, angles_, composition_per_atom, _ = self.decode_stats(
            z, num_atoms)
        ...
        if gt_atom_types is None:
            init_atom_types = self.sample_composition(
                composition_per_atom, num_atoms)
            
            cur_atom_types = init_atom_types
            print("Init atom type: ", cur_atom_types)

        else:
            cur_atom_types = gt_atom_types

        # init coords. # pytorch memory issue # overwrite cur_frac_coords creates new GPU memory usage
        ...
        for sigma in tqdm(self.sigmas, total=self.sigmas.size(0), disable=ld_kwargs.disable_bar):
            ...
            for step in range(ld_kwargs.n_step_each):
                ...
                batch_c_grad, batch_forces = self.compute_grad(crystal_graph_list= crystal_graph_list, classifier= classifier, ld_kwargs= ld_kwargs)
                ...
                # print("force induced displacement before clamping")
                # print(force_disp)
                
                # clamp to the maximum displacement of 0.5
                ...
                if change_type:
                    cur_atom_types = torch.argmax(pred_atom_types, dim=1) + 1    # init_atom_types # 
                else:
                    cur_atom_types = init_atom_types
    ...
    def forward(self, batch, teacher_forcing, training):
        ...
        (pred_num_atoms, pred_lengths_and_angles, pred_lengths, pred_angles,
         pred_composition_per_atom, pred_atom_volume) = self.decode_stats(
            z, gt_num_atoms=batch.num_atoms, gt_lengths=batch.lengths, gt_angles=batch.angles, 
            teacher_forcing = True)

        # (pred_num_atoms, pred_lengths_and_angles, pred_lengths, pred_angles,
        #  pred_composition_per_atom) = self.decode_volume(
        ...
        # init_type_probs = torch.tensor(F.one_hot(batch.atom_types - 1, num_classes=MAX_ATOMIC_NUM), 
        #                                device= self.device,
        #                                dtype= torch.float32)

        
        rand_atom_types = torch.multinomial(
            pred_composition_probs, num_samples=1).squeeze(1) + 1
    ...
    def predict_atom_volume(self, z):
        """
        volume per atom is a quantity that can be well described by GNN
        """
        return self.fc_atom_volume(z)
    ...
    def volume_loss(self, pred_atom_volume,  batch):
        # print(batch.atom_volume)
        return F.mse_loss(pred_atom_volume, batch.atom_volume)
    ...
    def compute_stats(self, batch, outputs, prefix):
        ...
        volume_loss = outputs['volume_loss']
        ...
        loss = (
            ...
            self.hparams.cost_volume * volume_loss +
        ...
        log_dict = {
            ...
            f'{prefix}_volume_loss': volume_loss,
    ...
    def on_train_epoch_end(self):
        ...
        print(f"Epoch {self.current_epoch} - volume_loss: {metrics.get('train_volume_loss_step', 0):.4f}")
