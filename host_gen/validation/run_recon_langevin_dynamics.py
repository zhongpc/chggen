...
from aux_funcs_recon import get_batch_data
...
# Load csv data.
data_recon = pd.read_csv('../data/dataset/data_recon.csv', keep_default_na=False, na_values=[''])
data_recon_new = data_recon.copy()
data_recon_new['cif_recon'] = ''

# Data loader parameter
batch_size = 32
num_total = len(data_recon)
...
for i in range(len(ptr) - 1):
    cur_atom_types, cur_frac_coords, num_atoms, angles, lengths = get_batch_data(data_recon, ptr[i], ptr[i+1], device)
    ...
    data_recon_new['cif_recon'][ptr[i]:ptr[i+1]] = cifs

    # Save to CSV every 5 minutes
    if time() - start_time > 1 * 60:
        print('Saving to CSV...')
        data_recon_new.to_csv('../data/data_recon.csv', index=False)
        start_time = time()
