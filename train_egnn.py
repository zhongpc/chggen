...
from chggen.common.data_utils import get_scaler, frac_score_norms_interp, get_exp_sigmas
...
train_dataset = CHGNetDataset(
    path= '/home/xzdai/ceder_group/material_dircovery/chggen_fully/data/perov_5/train.csv', # train.csv
    name = 'train_perov',
    prop_list = ['heat_all'],
)

val_dataset = CHGNetDataset(
    path= '/home/xzdai/ceder_group/material_dircovery/chggen_fully/data/perov_5/val.csv', # val.csv
    name = 'val_perov',
    prop_list = ['heat_all'],
)
...
# Compute normalize factor and save.
# sigmas_F = get_exp_sigmas(
#     sigma_begin = model_hparams['sigma_F_begin'],
#     sigma_end = model_hparams['sigma_F_end'],
#     num_noise_level = model_hparams['num_noise_level'],
# )
# frac_score_norms = frac_norm_factor_interp(
#     datamodule.train_dataloader(), sigmas=sigmas_F,
# )
# with open('./test_models/frac_score_norms_interp', 'wb') as fp:
#     pickle.dump(frac_score_norms, fp)
with open('./test_models/frac_score_norms_interp.pkl', 'rb') as fp:
    frac_score_norms = pickle.load(fp)

chggen = CHGGen(
    lattice_scaler = lattice_scaler, frac_score_norms = frac_score_norms,  hparams_dict = model_hparams
)
...
trainer.save_checkpoint("./test_models/perov/trainer.ckpt")
