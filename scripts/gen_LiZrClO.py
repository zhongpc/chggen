...
if __name__ == "__main__":
    ...
    for a in range(1, 3):
        for b in range(1, 4):
            comp_string = 'Li2ZrCl6'*a + 'Li2O'*b # + 'AlOCl'*c
            comp_list.append(comp_string)
    ...
    for comp_str in comp_list:
        for num_inpaint in range(args.num_iterations):
            ...
            df_, total_denovo, total_inpaint = main(csp= csp, chemical_formula= comp_str, atomic_volume = args.atomic_volume, 
                            species = args.species, # set the same to enable inpainting, set different to use as-generated structures
                            gen_kwargs = gen_kwargs, ld_kwargs = ld_kwargs)
            
            CURRENT_DATE = datetime.now().strftime("%Y-%m-%d")
            ROOT_DIR = './files/' + Composition(comp_str).reduced_formula
