...
def generate_crystal(fv_dict):
    ...
    for s_init, type_init in zip(s_init_list, init_type_list): # loop over different cubic lattice generated structures
        ...
        df = pd.DataFrame(columns=['material_id', 'formula', 'lattice_type', 'mutation', 
                                ...
                                's_init_cif', 'spacegroup_CG', 's_CG_frame_cif', 'E0_chgnet', 'E_chgnet']) # energy information from chgnet
        ...
        for mutation in range(gen_kwargs.num_mutation):
            ...
            try:
                ...
                allow_relax = True
                if allow_relax:
                    result = chg_relaxer.relax(atoms= atoms, fmax = 0.1, steps = 2000, relax_cell = True, verbose = True, )
                    s_relax = result["final_structure"]
                    toten = result['trajectory'].energies[-1]
                else:
                    s_relax = structure # result["final_structure"]
                    toten = E0_tot # result['trajectory'].energies[-1]\\

            except:
                print("Failed CHGNet prediction")
                continue
            ...
            for symprec, angle_tolerance in zip(sym_list, angle_list):
                s_frame = s_relax.copy()
                s_frame.remove_species(['Li'])
                analyzer_CG = SpacegroupAnalyzer(structure= s_frame, symprec= symprec, angle_tolerance= angle_tolerance)
                s_CG = analyzer_CG.get_conventional_standard_structure()
                symbol_CG= analyzer_CG.get_space_group_symbol()
                symbol_CG = symbol_CG.replace('/', '-')

                s_CG.to(filename = ROOT_DIR + '/s_CG_' + symbol_CG + '.cif' )
            ...
            save_dict = {'material_id': hex(int(time.time()*1e8)),
                         ...
                         'spacegroup_CG': symbol_CG,
                         's_CG_frame_cif': str(CifWriter(s_CG)),
                         'Fmax_chgnet': Fmax, 
                         'E0_chgnet': E0_tot,
                         'E_chgnet': toten,
...
if __name__ == '__main__':
    ...
    gen_kwargs = SimpleNamespace(
        ...
        num_mutation = 1, # number of mutations during the relax-generation iteration
